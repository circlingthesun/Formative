...
<%def name="script_section()">
    ...
    function init() {
      ...
      if(pic.width > 900){
        scale_factor = (900/pic.width);
        canvas.height = scale_factor*pic.height;
        canvas.width = 900;
      ...
      for(var i = 0; i < box_list.length; i++){
            ...
            addRect(box[0], box[1], box[2], box[3], colour);
      }
    ...
    function toJSON(b) {
      ...
      for (i = 0; i < b.length; ++i) {
        s += '[' + Math.round(b[i].x*scale) + ', ' +  Math.round(b[i].y*scale) + ', ' +  Math.round(b[i].w*scale) + ', ' +  Math.round(b[i].h*scale) + '], '; 
      }
      s += ']';
      return s;
    }
...
<form id="imgform" name="imgform" action="/new" method="post" enctype="multipart/form-data">
${renderer.csrf_token()}
<input id="boxes" name="boxes" type="hidden" />
